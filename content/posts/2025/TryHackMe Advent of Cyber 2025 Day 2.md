---
title: TryHackMe Advent of Cyber 2025 Day 2
summary: Walkthrough of the TryHackMe Advent of Cyber 2025 Day 2 - Phishing - Merry Clickmas.
date: 2025-12-05
categories:
  - walkthrough
tags:
  ...
  - AdventOfCyber2025
cover:
  image: /2025/aoc_2025_day_2/Advent_of_Cyber_2025_Day_2_cover.png
  hidden: false
...
Social Engineering is not always straightforward like that. Sometimes it's about manipulating the right user to obtain the right information, even the "harmless" ones, like calling the reception of a company and pretending I had a call from the Karen from HR and she didn't give me her last name. From that information, I have the full name and I can set up a spear phishing attack.
...
Phishing is one of the oldest tricks, and it's still here in 2025. It's evolving, especially with the rise of LLMs, attackers can now easily craft phishing emails with different tones. Nobody is 100% safe, not you, not me.
...
```shell-session
┌──(attacker㉿kali)-[~/Documents/AOC_2025]
└─$ ./server.py                            
...
This is basically what the victim is going to see when clicking on the malicious link.

![](/2025/aoc_2025_day_2/tbfc.png)

And when the victim sends their credentials, this is what we see as attackers : 

![](/2025/aoc_2025_day_2/phishing_script.png)
...
**Mini Spoiler Alert** By the way, if you watch Mr. Robot, you can see SEToolkit on multiple occasions; this screen is from Season 1, when Mobley found a way to trick Trudy at Steel Mountain.

![](/2025/aoc_2025_day_2/mr-robot_trudy.png) 
...
> - **SMTP email server address**: We will deliver directly to the TBFC mail server by entering `10.81.191.129`.
...
set:phishing> SMTP email server address (ex. smtp.youremailserveryouown.com):10.81.191.129
...
If you are working directly from your Kali machine, you may have a slightly different output using SEToolkit, if the tool is asking : 
...
You select `One-Time Use Email Template`.

## Questions

**What is the password used to access the TBFC portal?**

Once the phishing mail is sent, wait a little bit and credentials will appear on the phishing server : 

```shell-session
10.81.191.129 - - [05/Dec/2025 14:03:02] "POST /submit HTTP/1.1" 303 -
10.81.191.129 - - [05/Dec/2025 14:03:02] "GET / HTTP/1.1" 200 -
10.81.191.129 - - [05/Dec/2025 14:06:02] "GET / HTTP/1.1" 200 -
[2025-12-05 14:06:02] Captured -> username: admin    password: *******-******-******    from: 10.81.191.129
```

>Browse to `http://10.81.191.129` from within the AttackBox and try to access the mailbox of the `factory` user to see if the previously harvested `admin` password has been reused on the email portal. 

**What is the total number of toys expected for delivery?**
![](/2025/aoc_2025_day_2/mailbox.png)